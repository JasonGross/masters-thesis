...
\end{verbatim}
    ...
    \todoask{You said ``I don't understand where this rule oversimplifies, so maybe it is worth an explanation.''  Here's an explanation directed at you; is it worth including?}
    \newtext{This code, which matches our current implementation, doesn't quite allow us to handle what we claim to handle in this section; we should be able to handle any rule that starts with a nonterminal \nt{nt}, such that the set of possible characters in any string which can be parsed as \nt{nt} is disjoint from the set of possible first characters of any string parsed by the rest of the rule.  This code only handles the case where the rest of the rule begins with a terminal.}
