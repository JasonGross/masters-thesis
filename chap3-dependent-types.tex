...
\section{Minimal Parse Trees: Formal Definition} \label{sec:min:formal-definition}
  ...
  \begin{align*}
    ...
    & \typeto \indname{MinItemsTreeOf}~~\farg{s$_0$}~~(\farg{unseen} - \{\farg{nt}\})~~\farg{its}~~\farg{s} \\
    & \typeto \indname{MinParseTreeOf}~~\farg{s$_0$}~~\farg{unseen}~~\farg{nt}~~\farg{s}
  \end{align*}
...
\begin{figure*} \caption{The dependently typed interface of our parser, Part 1 of 2}\label{fig:parser-interface}
...
\editedtext[3]{\begin{align*}
  ...
  \fname{terminal\_failure} & \oftypesep \forall~\farg{ch}~\farg{s},~\farg{s} \ne \str{\farg{ch}}
  ...
  \fname{cons\_success} & \oftypesep (\oftype{\farg{it}}{\indname{Item}})
   ...
   \typeto (\oftype{\farg{s$_1$}~\farg{s$_2$}}{\String}) \\
  ...
  \fname{cons\_failure} & \oftypesep (\oftype{\farg{it}}{\indname{Item}})
   ...
   & \phantom{{}\typeto\left(\forall\vphantom{\parsetreetype{}{}}{}\right.}~~
     \fname{T$_{\fname{failure}}$}~~\farg{s$_0$}~~\farg{unseen}~~\left(\parsetreetype{\farg{it}}{\farg{s$_1$}}\right) \\
   & \phantom{{}\typeto\left(\forall\vphantom{\parsetreetype{}{}}{}\right.}~~\typesumsep {\fname{T$_{\fname{failure}}$}~~\farg{s$_0$}~~\farg{unseen}~~\left(\parsetreetype{\farg{its}}{\farg{s$_2$}}\right)}\big) \\
...
\end{align*}}
\end{figure*}
\begin{figure*}\caption{The dependently typed interface of our parser, Part 2 of 2}\label{fig:parser-interface-2}
\editedtext[3]{\begin{align*}
...
\begin{figure*}\caption{Pseudo-Implementation of our parser.  We \editedtext{adopt} the convention that dependent indices to functions (e.g., \fname{unseen}) are implicit.}\label{fig:parser-impl}%
  \editedtext[3]{\begin{align*}
    ...
    & \qquad \cthen\cif\left(\fname{parse$'$}~~(\farg{s$_0$}\defeq\farg{s})~~(\farg{unseen}\defeq\fname{unseen$_0$})~~(\parsetreetype{\farg{its}}{\farg{s}})\right)~~\text{succeeds} \\
    & \qquad\cifindent\cifindent\qquad\text{ returning}~~\farg{d}~~\text{for any production \farg{p} mapping \farg{nt} to \farg{its}}  \\
    ...
    & \qquad\cifindent\cthen\cif\left(\fname{parse$'$}~~(\farg{unseen}\defeq\fname{unseen} - \{\farg{nt}\})~~(\parsetreetype{\farg{its}}{\farg{s}})\right)~~\text{succeeds} \\
    & \qquad\cifindent\cifindent\cifindent\text{returning}~~\farg{d}~~\text{for any production \farg{p} mapping \farg{nt} to \farg{its}}  \\
    ...
    & \qquad\caseof{\fname{parse$'$}~~(\parsetreetype{\farg{it}}{\fname{take}\ensuremath{{}_{\farg{x}}}~~\farg{s}})
        ...
        \textbf{,} \\ & \qquad\phantom{\precaseof}\fname{any\_parse}~~\farg{s}~~\farg{it}~~\farg{its}~~\farg{xs}} \\
  ...
  {\hfill $\forall~\farg{x$'$} \in \left(\cons{\farg{x}}{\farg{xs}}\right),~~\typesum{\fname{T$_{\fname{failure}}$}~~\termhole~~\termhole~~(\parsetreetype{\farg{it}}{\fname{take}_{\farg{x$'$}}~~\farg{s}})}{\fname{T$_{\fname{failure}}$}~~\termhole~~\termhole~~(\parsetreetype{\farg{its}}{\fname{drop}_{\farg{x$'$}}~~\farg{s}})}$\hfill} \\
...
%    \end{center}
  ...
  \subsection{Parametricity} \label{sec:parser-extensionality-theorem}
    ...
    \end{align*}
    where \fname{bool\_of\_sum} is, for any types $A$ and $B$, the function of type \typesum{\ensuremath{A}}{\ensuremath{B}}~\typeto~\Bool\space obtained by sending everything in the left component to \true\space and everything in the right component to \false.
...
\section{Semantic Actions} \label{sec:semantic-actions}
  \newtext[2]{Our parsing algorithm can also be specialized to handle the common use case of semantic actions.  Consider, for example, the following simultaneous specification of a grammar and some semantic actions:}%
  \newtext[2]{\begin{align*}
  ...
  \end{align*}}%
  \newtext[2]{Supposing we have defined this grammar separately for our parser, we can instantiate the interface as follows to implement these semantic actions:}%
  \newtext[2]{\begin{alignat*}{2}
    ...
    & \fname{T$_{\fname{failure}}$}~~\termhole~~\termhole~~\termhole  && \defeq \Unit
  \end{alignat*}}%
  \newtext[2]{As all \fname{failure} cases are instantiated with \unittt, we elide them.}%
  \newtext[2]{\begin{align*}
  ...
  \end{align*}}%
  \newtext[2]{We will use a single definition definition \fname{production\_success} to combine \fname{production\_success$_<$} and \fname{production\_success$_=$} here, as the definition does not depend on any of the arguments that vary between them.  This is where the semantic actions take place.  We deal first with the case of a number:}%
  \newtext[2]{\begin{align*}
