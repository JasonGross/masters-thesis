...
\section{Optimizations}
  ...
  \subsection{Upcoming Optimizations}
    ...
    For each grammar, the Fiat framework presents us with goals describing the unimplemented portion of the splitter for this particular grammar.  For example, the goal for the \regex{(ab)*} grammar looks like this: \todo{<INSERT GOAL>}.  To get to this goal, we write this code: \todo{<COQ CODE HERE, with comments describing what each line does>}  We thus have to describe how to split a string for the rules \todo{<RULES HERE>}, and provide proofs that these splitting strategies are complete.  We begin the next section with this splitting strategy.
