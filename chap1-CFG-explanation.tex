...
\section{Parsing}
  ...
  \subsection{Infinite Regress} \label{sec:loopy-grammar-example}
    Since we have programmed our parser in Coq (see \autoref{sec:coq}),\todo{Add intro about Coq} \todoask{Where should the intro about Coq go?}, our program must be terminating by construction.  However, naive recursive-descent parsers do not always terminate!
...
\section{Standard Formal Definitions} \label{sec:standard-definitions}
  ...
  \subsection{Context-Free Grammar}
    ...
    As in standard presentations, we restrict our attention to grammars where the set of nonterminals is finite.  In our formalization, since a nonterminal is named by a string, we require that each grammar provide a list of ``valid'' nonterminals. \todoask{Should I mention the closure condition, that each production of a valid nonterminal can only mention valid nonterminals?  Does this belong here, or elsewhere?}
